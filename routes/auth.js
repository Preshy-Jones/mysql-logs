...
router.get('/createdb', (req, res) => {
    ...
    db.query(sql, (err, result) => {
        if (err) throw err;
        console.log(result);
        res.status.json({ msg: 'Database created ...' });
    })
})
...
router.get('/createuserstable', (req, res) => {
    ...
    db.query(sql, (err, result) => {
        if (err) throw err;
        console.log(result);
        res.status.json({ msg: 'Users table created....' });
    });
});
...
router.get('/populate', (req, res) => {
    request({
        ...
        res.status(200).json({ msg: 'users data added....' })
    });
...
router.get('/delete', (req, res) => {
    ...
    let query = db.query(sql, (err, result) => {
        if (err) throw err;
        console.log(result);
        res.status(200).json({ msg: 'User in last row deleted....' });
